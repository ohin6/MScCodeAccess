...
#* include
        ...
        #* 6. Schwartzentruber
#* 
#* eQTL data examines gene expresssion of HLA-DRB1, HLA-DQA1 and HLA-DRA in 
...
HLA_expression_tissue = function(tissue){
  studies_df = dplyr::filter(tabix_paths, study == "GTEx", tissue_label == tissue)
...
adiposeVisceral = HLA_expression_tissue("adipose (visceral)")
...
# Marioni_2018 ------------------------------------------------------------
# Get GWAS HLA region
Maroni_HLA_loci = data.table::fread('../Raw_data/Marioni_2019.txt', header = T) %>%
...
Maroni_HLA_loci2 <- Maroni_HLA_loci[order(BP),] %>%
  filter(BP %in% sumStats$BP) %>%
  dplyr::filter(P<0.007)

sumStats =  sumStats[order(BP),] %>%
  filter(BP %in% Maroni_HLA_loci2$BP)
...
LD = LDlinkR::LDmatrix(snps = Maroni_HLA_loci2$SNP, pop = "EUR", r2d = "r2",
...
Maroni_HLA_loci2 = Maroni_HLA_loci2 %>%
...
LD.x = LD[-1]
# rownames = colnmaes
rownames(LD.x) = Maroni_HLA_loci2$BP
...
D2 = list(beta = Maroni_HLA_loci2$BETA,
          varbeta = Maroni_HLA_loci2$SE*Maroni_HLA_loci2$SE,
          SNP = Maroni_HLA_loci2$BP,
          position = Maroni_HLA_loci2$BP,
          type = 'cc',
          LD = LD.x,
          sdY = 1)
...
zscore2 = Maroni_HLA_loci2$BETA/Maroni_HLA_loci2$SE
...
################################################################################
######################## Repeat for different GWAS data ########################
################################################################################
